...
export default function Home() {
  ...
  const handleNextVideo = () => {
    setCurrentVideoIndex((prev) =>
      videos.length ? (prev + 1) % videos.length : 0
    );
  };

  const handlePrevVideo = () => {
    setCurrentVideoIndex((prev) =>
      videos.length ? (prev - 1 + videos.length) % videos.length : 0
    );
  };
